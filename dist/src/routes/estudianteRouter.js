...
const estudianteController = __importStar(require("../controllers/estudianteControllers"));
...
// Ruta para obtener un estudiante por su ID (GET)
...
// Ruta para actualizar un estudiante por su ID (PUT)
estudianteRouter.put('/:cod_e', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const cod_e = parseInt(req.params.cod_e);
    const updatedEstudiante = req.body;
    estudianteController.put(cod_e, updatedEstudiante, (err, result) => {
        if (err) {
            return res.status(500).json({ message: err.message });
        }
        res.status(result.statusCode).json(result);
...
// Ruta para eliminar un estudiante por su ID (DELETE)
